...
from .types import DiffChange, DiffResult, DiffSummary, KBContent
...
class KBDiffService:
    ...
    def generate_changes(self, current_content: KBContent, scraped_content: KBContent) -> DiffResult:
        """
        ...
        # Compare FAQs
        faq_changes = self._diff_faqs(current_content.get("faqs", []), scraped_content.get("faqs", []))
        ...
        pdf_changes = self._diff_documents(
            current_content.get("pdf_documents", []), scraped_content.get("pdf_documents", [])
        )
        changes.extend(pdf_changes)

        # Compare arrangements
        arrangement_changes = self._diff_arrangements(
            current_content.get("arrangements", []), scraped_content.get("arrangements", [])
        )
        changes.extend(arrangement_changes)

        # Compare business info
        business_changes = self._diff_business_info(
            current_content.get("business_info", {}), scraped_content.get("business_info", {})
        )
    ...
    def apply_changes(
        self, current_content: KBContent, changes_to_apply: list[str], all_changes: list[DiffChange]
    ) -> KBContent:
        """
    ...
    def _diff_faqs(self, current: list[dict], scraped: list[dict]) -> list[DiffChange]:
        ...
        matched_pairs, added, removed = self._match_items_by_exact_then_similarity(
            current, scraped, key_field="question", threshold=0.90
        )

        # Check for modifications in matched pairs
        for curr_item, new_item, curr_idx, _new_idx in matched_pairs:
            modifications = []

            if curr_item.get("question") != new_item.get("question"):
                modifications.append(
                    {"field": "question", "old_value": curr_item.get("question"), "new_value": new_item.get("question")}
                )

            if curr_item.get("answer") != new_item.get("answer"):
                diff_html = self._generate_text_diff(curr_item.get("answer", ""), new_item.get("answer", ""))
                modifications.append(
                    {
                        "field": "answer",
            ...
            if curr_item.get("category") != new_item.get("category"):
                modifications.append(
                    {"field": "category", "old_value": curr_item.get("category"), "new_value": new_item.get("category")}
                )
    ...
    def _diff_sections(self, current: list[dict], scraped: list[dict]) -> list[DiffChange]:
        ...
        matched_pairs, added, removed = self._match_items_by_field(current, scraped, key_field="url")
        ...
        for curr_item, new_item, curr_idx, _new_idx in matched_pairs:
            ...
            if curr_item.get("title") != new_item.get("title"):
                modifications.append(
                    {"field": "title", "old_value": curr_item.get("title"), "new_value": new_item.get("title")}
                )

            if curr_item.get("content") != new_item.get("content"):
                diff_html = self._generate_text_diff(
                    curr_item.get("content", "")[:500], new_item.get("content", "")[:500]
                )
                modifications.append(
                    {
                        "field": "content",
    ...
    def _diff_documents(self, current: list[dict], scraped: list[dict]) -> list[DiffChange]:
        ...
        matched_pairs, added, removed = self._match_items_by_field(current or [], scraped or [], key_field="url")
        ...
        for curr_item, new_item, curr_idx, _new_idx in matched_pairs:
            ...
            if curr_item.get("filename") != new_item.get("filename"):
                modifications.append(
                    {"field": "filename", "old_value": curr_item.get("filename"), "new_value": new_item.get("filename")}
                )
    ...
    def _diff_arrangements(self, current: list[dict], scraped: list[dict]) -> list[DiffChange]:
        ...
        matched_pairs, added, removed = self._match_items_by_exact_then_similarity(
            current, scraped, key_field="name", threshold=0.92
        )
        ...
        for curr_item, new_item, curr_idx, _new_idx in matched_pairs:
            ...
            for field in ["name", "description", "duration", "category", "age_restriction", "group_size"]:
                if curr_item.get(field) != new_item.get(field):
                    modifications.append(
                        {"field": field, "old_value": curr_item.get(field), "new_value": new_item.get(field)}
                    )
    ...
    def _match_items_by_field(
        self, current: list[dict], scraped: list[dict], key_field: str
    ...
    def _match_items_by_exact_then_similarity(
        self, current: list[dict], scraped: list[dict], key_field: str, threshold: float = 0.85
    ) -> tuple[list, list, list]:
        """
    ...
    def _generate_summary(self, changes: list[DiffChange]) -> DiffSummary:
        ...
        for change in changes:
            ...
            if change_type not in summary["by_type"]:
                summary["by_type"][change_type] = {"ADDED": 0, "MODIFIED": 0, "REMOVED": 0}
...
_diff_service_instance: KBDiffService | None = None
